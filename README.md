...
| :--- | :--- | :--- | :--- |
| **`=== [ 1. PURCHASE LIMITS & RESET ] ===`** | `string` | `""` | *Section Divider: Purchase limits and reset cycle* |
...
| **`=== [ 2. PRICE ESCALATION ] ===`** | `string` | `""` | *Section Divider: Price increase per buy* |
...
| **`=== [ 3. BOARD ACTIVATION CONDITIONS ] ===`** | `string` | `""` | *Section Divider: Prerequisites before mod activates* |
...
| **`Condition: Card ID to Count`** | `string` | `villager` | Card ID(s) to count (e.g., `villager`, `coin`, or comma-separated `villager, militia`). Supports `villager` and `vilager`. Leave blank or `*` to count all cards on board. |
| **`Condition: Required Card Count`** | `int` | `3` | Minimum number of matching cards required to activate the mod. Must be 1 or higher. |
...
| **`=== [ 4. SAVE ACTIONS ] ===`** | `string` | `""` | *Section Divider: Reset and save tools* |
